                        ...
                        case "ValueList":
                            {
                                ...
                                foreach (KeyValuePair<string, List<ResthopperObject>> entree in tree)
                                {
                                    for (int i = 0; i < entree.Value.Count; i++)
                                    {
                                        ...
                                        string data;
                                        try { data = JsonConvert.DeserializeObject<string>(restobj.Data) ?? restobj.Data.Trim('"'); }
        ...
        private static object SerializeDataTree(IGH_Structure data, Guid paramId, string name, int rhinoVersion = 7)
        {
            ...
            foreach (var path in data.Paths)
            {
                ...
                foreach (var goo in data.get_Branch(path))
                {
                    ...
                    // Selva SDK seam: any Goo implementing ISelvaSerializableGoo (matched by interface
                    // name, not assembly reference) owns its compute JSON. The wire Type stays the
                    ...
                    // DEPRECATED — DELETE THIS BLOCK in a future major. Serializes Selva output Goos from
